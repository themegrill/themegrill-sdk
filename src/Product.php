<?php
/**
 * The product model class for ThemeGrill SDK
 *
...
/**
 * Product model for ThemeGrill SDK.
 */
class Product {
	/**
	 ...
	 * Root api endpoint.
	 */
	const API_URL = 'https://api.themegrill.com/';

	/**
	 * Themegrill_sdk_Product constructor.
	 *
	...
	public function __construct( $basefile ) {
		...
		if ( 0 === $install ) {
			 ...
			 */
			do_action( 'themegrill_sdk_first_activation', $basefile );
		...
		if ( $current_version !== $this->version && wp_cache_get( "{$this->slug}_version_upgrade" ) === false ) {
			 ...
			 * @param string $basefile The basefile of the product.
			 */
			do_action( "themegrill_sdk_update_{$this->slug}", $current_version, $this->version, $basefile );
	...
	public function get_friendly_name() {
		$name = apply_filters( $this->get_key() . '_friendly_name', $this->get_name() );
		return $name;
	}

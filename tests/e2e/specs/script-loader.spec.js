...
test.describe('Script Loader', () => {
	test('check survey and tracking script loading (on Visualizer)', async ({
		page,
		admin,
		context,
	}) => {
		...
		await expect(page.locator('script#themegrill_sdk_survey_script-js')).toHaveCount(1);
		await expect(page.locator('script#themegrill_sdk_tracking_script-js')).toHaveCount(1);
		await expect(page.locator('script#themegrill_sdk_tracking_script-js-extra')).toHaveCount(1);
